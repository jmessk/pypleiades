...
async def main():
    ...
    spawner_task = asyncio.create_task(worker_spawner(task_queue))
    contractor_task = asyncio.create_task(
        contranctor(worker.unwrap().worker_id, task_queue)
    )

    await asyncio.gather(contractor_task, spawner_task)
    print("Down main")

# contranctor
async def contranctor(worker_id: str, job_queue: asyncio.Queue):
    while not q_flag:
